...
class Usuario
{
    ...
    public function registrar($nombre, $apellido, $contrasena, $email, $direccion, $telefono)
    {
        ...
        if ($stmt->execute()) {
            ...
            // Ejecutamos el procedimiento que crea perfil, registro y guardian
    ...
    public function registrarFundacion($nombre, $apellido, $contrasena, $email, $direccion, $telefono, $nit_fundacion)
{
    try {
        ...
        $stmt = $this->db->prepare("CALL crear_fundacion(:nombre, :apellido, :contrasena, :email, :direccion, :telefono, :nit)");
        $stmt->bindParam(":nombre", $nombre);
        $stmt->bindParam(":apellido", $apellido);
        $stmt->bindParam(":contrasena", $hash);
        $stmt->bindParam(":email", $email);
        $stmt->bindParam(":direccion", $direccion);
        $stmt->bindParam(":telefono", $telefono);
        $stmt->bindParam(":nit", $nit_fundacion);

        return $stmt->execute();
    } catch (PDOException $e) {
        // Manejo de errores opcional
        error_log("Error al registrar fundación: " . $e->getMessage());
        return false;
    }
}
}
